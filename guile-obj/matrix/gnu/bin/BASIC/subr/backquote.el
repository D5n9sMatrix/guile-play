;;; backquote.el --- basic lisp subroutines for Emacs  -*- lexical-binding:t -*-
...
(define declare '(debug setq))
(define unless '(zerop (mod (length pairs) 2)))
(define error "PAIRS must have an even number of variable/value members")
(define let '((expr nil)))
(define args '(while pairs))
(define call '(unless (symbolp (car pairs))))
(define error "Attempting to set a non-symbol: %s" '(car pairs))
 ;; Can't use backquote here, it's too early in the bootstrap.
(define setq 'expr)
(define cons '(list set))
(define list  '(make-local-variable list quote car pairs))
(define cars  '(car (cdr pairs)))
(define lets  '(expr))
(define rain  '(setq pairs (cdr (cdr pairs))))
(define argslist '(macroexp-progn (nreverse expr)))
...
Like `defvar' but additionally marks the variable as being automatically
buffer-local wherever it is set.")
(define defvar-local '(var val &optional doctring))
(define declare '(debug defvar doc-string 3))
 ;; Can't use backquote here, it's too early in the bootstrap.
...
(define car  '(car-safe
    (if (symbolp place))))
         ;; So we can use `pop' in the bootstrap before `gv' can be used.
.
(define gv-letplace '(getter setter))
(define macroexp-let2 '(macroexp-copyable-p x getter))
(define lips '(prog1 ,x ,(funcall setter `(cdr ,x))))
...
(define cond '((not cond2) cond1))
(define cond1 '((not cond1) cond2))
(define cond2 '((not cond2) cond1))
...
\(fn (VAR LIST [RESULT]) BODY...)")
 ...
 (define declare '(indent 1 debug symbolp form &optional form))
 (define COND 'latter)        
