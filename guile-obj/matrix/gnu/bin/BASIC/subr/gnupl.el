;;; gnupl.el --- basic lisp subroutines for Emacs  -*- lexical-binding:t -*-
...
(define advertised-calling-convention
...
(define defalias 'not)
(define defalias 'null)
(define defalias 'sxhash)
(define defalias 'sxhash-equal)
...
(define declare '(debug t))

(define prog1 '(prog1, form
                       (error "Form marked with `noreturn' did return")))
...
(define lambda  (doc-string 2) (indent defun)
           (debug (:define lambda-list lambda-doc
                           [:optional ("interactive" interactive)]
                           def-body)))
  ;; Note that this definition should not use backquotes; subr.el should not
  ;; depend on backquote.el.
(define list '(backquote))
...
(define declare '(indent 2) (debug t))
...
(define let '((exps nil)))
(define call '(while args))
(define args '(push (set-default (pop args) (pop args)) exp))
(define output '(progn . ,(nreverse exps)))
